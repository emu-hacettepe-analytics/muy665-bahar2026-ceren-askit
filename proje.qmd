...
[Pakize Hergül Kaya](takim/pakizehergulkaya.qmd){.download-button}
[Esin Gül](takim/esingul.qmd){.download-button}
[Baran Balcı](takim/baranbalci.qmd){.download-button}
...
internet_kullanim <- read_excel("data/CinsiyeteGoreBireylerinİnternetKullanımOranı.xlsx")
...
knitr::kable(internet_kullanim[1:5, ])
...
#internet kullanım
internet_ozet <- data.frame(
  Sutun_Adi = colnames(internet_kullanim),
  NA_Sayisi = colSums(is.na(internet_kullanim))
)
knitr::kable(internet_ozet, caption = "Sütun İsimleri ve Eksik Veri (NA) Sayıları")
...
ggplot(eglence_nufus_data, aes(x = Nufus_Milyon, y = Eglence_Kultur_Harcama)) +
  ...
  labs(
    title = "Nüfus Dinamiklerinin Eğlence ve Kültür Harcamalarına Etkisi",
...
```{r eglencekultur-nüfus-kultur-dataframe, message=FALSE, warning=FALSE, echo=FALSE}
